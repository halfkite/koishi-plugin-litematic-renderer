...
- 缓存按投影 SHA-256 分目录，同内容复用；统一目录包含原始投影、固定名称 PNG、`about.json5` 和 `index.json5`
- 渲染诊断持久化，可导出错误报告与诊断日志包
- GPU Agent 0.4.4 增加 Runtime 进程树回收、队列内存上限和不中断队列的内存保护，详见 [Agent 使用文档](./gpu-render-agent/USAGE-0.4.4.md)
